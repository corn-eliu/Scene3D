## from https://github.com/mrbaozi/triangulation
def IsConvex(a, b, c):
...
## changed this to keep of which indices in the poly list are used and which index in the index list, each
## point has
def GetEar(poly, indices, availableIndices, doReturnIndices=False):
	size = len(poly)
	if size < 3:
		return []
	if size == 3:
		...
		del poly[:]
		needReverse = IsClockwise(list(tri))
		if doReturnIndices :
			result = (indices[availableIndices][0],
				  indices[availableIndices][1],
				  indices[availableIndices][2])
			availableIndices[indices[availableIndices][0]] = False
			if needReverse :
				return result[::-1]
			else :
				return result
	...
	for i in range(size):
		...
		if IsConvex(p1, p2, p3):
			...
			if tritest == False:
				del poly[i % size]
				needReverse = IsClockwise([p1, p2, p3])
				if doReturnIndices :
					result = (indices[availableIndices][(i-1) % size],
						  indices[availableIndices][i % size],
						  indices[availableIndices][(i+1) % size])
					availableIndices[indices[availableIndices][i % size]] = False
					if needReverse :
						return result[::-1]
					else :
						return result
